...
const savedCards = [
  { id: 1, mask: "**** **** **** 4521", bank: "Сбербанк", color: "from-green-600 to-green-800" },
  { id: 2, mask: "**** **** **** 7834", bank: "Тинькофф", color: "from-yellow-500 to-yellow-700" },
];
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-4 animate-fade-in">
        <div className="w-20 h-20 rounded-full bg-nd-green/10 flex items-center justify-center mb-6">
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="px-4 pt-10 pb-5">
        <h1 className="text-2xl font-black text-foreground">Вывод средств</h1>
        <p className="text-nd-muted text-sm mt-1">Переводим деньги напрямую на вашу карту</p>
      </div>
      ...
      <div className="px-4 mb-5">
        <div className="bg-nd-card nd-card-glow rounded-2xl p-4 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-nd-green/10 flex items-center justify-center">
      ...
      {/* Amount input */}
      <div className="px-4 mb-5">
        <p className="text-nd-muted text-xs font-medium uppercase tracking-wider mb-2">Сумма вывода</p>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <input
                ...
                max={available}
                className="w-full bg-transparent text-foreground text-3xl font-black outline-none placeholder:text-nd-border"
                placeholder="0"
              />
            </div>
      ...
      {/* Card selection */}
      <div className="px-4 mb-5">
        <p className="text-nd-muted text-xs font-medium uppercase tracking-wider mb-2">Карта для вывода</p>
        <div className="space-y-2">
          {savedCards.map((card) => (
            <button
              key={card.id}
              onClick={() => setSelectedCard(card.id)}
              className={`w-full bg-nd-card nd-card-glow rounded-2xl p-4 flex items-center gap-3 transition-all ${
                selectedCard === card.id ? "border border-nd-red" : "border border-transparent"
              }`}
            >
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                selectedCard === card.id ? "border-nd-red bg-nd-red" : "border-nd-border"
              }`}>
                {selectedCard === card.id && <Icon name="Check" size={10} className="text-white" />}
              </div>
            </button>
          ))}

          <button className="w-full bg-nd-card nd-card-glow rounded-2xl p-4 flex items-center gap-3 border border-dashed border-nd-border">
      ...
      {/* Info */}
      <div className="px-4 mb-5">
        <div className="bg-nd-card2 rounded-xl p-3 flex gap-2">
          <Icon name="Info" size={14} className="text-nd-muted flex-shrink-0 mt-0.5" />
      ...
      <div className="px-4 mb-8">
        <button
          ...
          className="w-full py-4 rounded-2xl bg-nd-red text-white font-bold text-base nd-red-glow hover-scale disabled:opacity-50 disabled:cursor-not-allowed transition-all"
        >
